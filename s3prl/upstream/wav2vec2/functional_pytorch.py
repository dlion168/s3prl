...
import sys
sys.path.insert(0, '/opt/conda/envs/s3prl_old_cuda/lib/python3.9/site-packages/torch/nn')
...
def multi_head_attention_forward(
    ...
    pre_wo_output = attn_output.transpose(0, 1).contiguous().view(tgt_len, bsz, embed_dim)
    attn_output = linear(pre_wo_output, out_proj_weight, out_proj_bias)
    ...
    if need_weights:
        ...
        return attn_output, attn_output_weights, pre_wo_output
    else:
        if not is_batched:
            # squeeze the output if input was unbatched
            attn_output = attn_output.squeeze(1)
        return attn_output, None, pre_wo_output
