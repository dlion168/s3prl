...
class MTTAudioDataset(data.Dataset):
...
class MTTFeatureDataset(data.Dataset):
    def __init__(self, feature_dir, datarc, split, return_audio_path=True, split_version=0, low_quality_source=True): # Singer trim?
        # self.cfg = cfg
        if split == 'dev':
            split = 'validation'
        self.metadata = pd.read_csv(os.path.join(datarc['meta_data'], f'{split}.tsv'), sep='\t', names = ['uuid', 'audio_path'])
