...
class DownstreamExpert(nn.Module):
    ...
    def apply_debiasing(self, method):
        ...
        for idx in range(len(self.train_dataset)):
            ...
            class_gender_pairs.append((class_idx, gender[-1], idx))
        
        dev_class_gender_pairs = []
        for idx in range(len(self.dev_dataset)):
            wav, lab, utt, gender = self.dev_dataset[idx]
            class_idx = np.argmax(lab)
            dev_class_gender_pairs.append((class_idx, gender[-1], idx))
        ...
        if method == "DS":
            class_female = defaultdict(list)  # {class_c: [(pF, idx), ...]}
            class_male   = defaultdict(list)  # {class_c: [(pM, idx), ...]}

            for idx in range(train_len):
                wav, lab, utt, gender = self.train_dataset[idx]
                c = np.argmax(lab)  # emotion class
                # gender = [pF, pM, pC, pseudo]
                pF, pM, _, _, _ = gender  
                class_female[c].append((pF, idx))
                class_male[c].append((pM, idx))
            ...
            for c, gender_dict in class_genders.items():
                fem_list = sorted(class_female[c], key=lambda x: x[0], reverse=True)
                mal_list = sorted(class_male[c],   key=lambda x: x[0], reverse=True)

                # 只取 female, male 間的最小長度
                min_count = min(len(idx_list) for idx_list in gender_dict.values())
                fem_top = [item[1] for item in fem_list[:min_count]]
                mal_top = [item[1] for item in mal_list[:min_count]]
                new_indices.extend(fem_top + mal_top)
    ...
    def forward(self, mode, features, labels, filenames, gender_labels, sample_weights, records, **kwargs):
        ...
        # -------------------- 其他模式: GroupDRO ---------------------------
        elif self.training_mode == "PGroupDRO":
            per_sample_loss = self.objective(
                logits_debiased, 
                labels, 
                self.class_balanced_weights.to(device), 
            ...
            # Corrected the tensor creation and reshaping for gender_soft_labels
            gender_soft_labels = torch.stack(
                [gender_labels[:, 1], gender_labels[:, 0]], dim=1
            ).to(device)  # 0: male, 1: female
            # Corrected the unsqueezing and broadcasting dimensions
            loss_g_list = per_sample_loss.unsqueeze(1) * gender_soft_labels
            print(loss_g_list)
            loss_g_list = loss_g_list.mean(dim=0)
            # Ensuring loss_regularize is on the same device and in the correct shape
            loss_regularize = torch.FloatTensor(
                [self.lambda_GDRO / math.sqrt(self.gender_count[g]) for g in [0, 1]]
            ).to(device)

            # Broadcasting addition
            loss_g_list = loss_g_list + loss_regularize

            # Use torch.max instead of max for tensors
            worst_L_g, worst_idx = torch.max(loss_g_list, dim=0)

            total_loss = worst_L_g   # Convert tensor to scalar for loss aggregation
        ...
        # -------------------- 其他模式: GR ---------------------------
            ...
            for c in range(C):
                ...
                for g in unique_genders:
                    g_int = int(g.item())
                    mask = (gender_hard_labels == g).float()
            ...
            1) 取樣本所屬 cluster (由 dataset 提供的 pseudo label)
            2) 對每個 cluster 計算 batch 裡的平均 loss
            3) 以 EMA 更新 cluster_importance[cluster_id]
            4) 將所有 cluster_importance 在本 batch 內 normalize
            5) 依照對應 cluster 給每個 sample 一個權重 alpha_i
            6) 總 loss = Σ_i ( alpha_i * per_sample_loss_i )
            """
            # Step1: 取得 cluster_id (假設存於 gender_labels[:, -1])
            cluster_ids = gender_hard_labels
            unique_clusters = torch.unique(cluster_ids)

            # Step2: 計算每個 cluster 的平均 loss
            cluster_loss_map = {}
            for c in unique_clusters:
                mask = (cluster_ids == c)
                if mask.sum() == 0:
                    continue
                avg_loss_c = per_sample_loss[mask].mean()  # 這個 cluster 的平均 loss
                cluster_loss_map[c.item()] = avg_loss_c.detach()

            # Step3: EMA 更新 cluster_importance[c]
            #   w_c <- alpha * w_c + (1-alpha) * cluster_loss
            for c, avg_loss_c in cluster_loss_map.items():
                old_w = self.cluster_importance[c]  # default=0.0 if not exist
                new_w = self.bpa_ema_alpha * old_w + (1 - self.bpa_ema_alpha)* avg_loss_c.item()
                self.cluster_importance[c] = new_w  # 更新

            # 如果本 batch 有新的 cluster，但沒有計算到 loss => 會保持0
            # 之後再下一個 batch 若該 cluster 出現，就會填進來

            # Step4: normalize
            #   sum_w = Σ_{all c in batch} self.cluster_importance[c]
            #   取 batch 裡實際出現的 cluster 做歸一化
            sum_w = 0.0
            for c in unique_clusters:
                sum_w += self.cluster_importance[c]

            # 避免 sum_w=0 的情況 (假如全部都還是初始0 或 batch非常小)
            if sum_w < 1e-8:
                # fallback: 全部給 uniform=1
                sample_weight_bpa = torch.ones_like(per_sample_loss, device=device)
            else:
                # alpha_i = cluster_importance[c_i] / sum_w
                sample_weight_bpa = torch.zeros_like(per_sample_loss, device=device)
                for c in unique_clusters:
                    mask = (cluster_ids == c)
                    w_c = self.cluster_importance[c] / sum_w
                    sample_weight_bpa[mask] = w_c

            # Step5: final loss
            # 先乘上 sample_weights (原先 DS/RW 可能會改變)
            final_weight_each = sample_weight_bpa * sample_weights
            total_loss = (final_weight_each * per_sample_loss).sum()  # or .mean(), 依需求
            predicted_logits = logits_debiased
        else:
