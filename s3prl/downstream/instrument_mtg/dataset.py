...
class MTGInstrumentAudioDataset(data.Dataset):
    def __init__(self, audio_dir, datarc, split, return_audio_path=True, split_version=0, low_quality_source=True): # Singer trim?
        # self.cfg = cfg
        if split == 'dev':
            split = 'validation'
        self.metadata_dir = os.path.join(datarc['meta_data'], f'data/splits/split-{split_version}/autotagging_instrument-{split}.tsv')
    ...
    def read_class2id(self, metadata_dir, split_version):
        ...
        for split in ['train', 'validation', 'test']:
            data = open(os.path.join(metadata_dir, f'data/splits/split-{split_version}/autotagging_instrument-{split}.tsv'), "r").readlines()
...
class MTGInstrumentFeatureDataset(data.Dataset):
