...
#-------------#
from ..model import *
from .dataset import MTGGenreAudioDataset, MTGGenreFeatureDataset
...
class DownstreamExpert(nn.Module):
    ...
    def __init__(self, upstream_dim, downstream_expert, expdir, **kwargs):
        ...
        self.train_dataset = MTGGenreFeatureDataset(self.pre_extract_dir, self.datarc, 'train') if self.pre_extract_dir else MTGGenreAudioDataset(root_dir, self.datarc, 'train')
        self.dev_dataset = MTGGenreFeatureDataset(self.pre_extract_dir, self.datarc, 'dev') if self.pre_extract_dir else MTGGenreAudioDataset(root_dir, self.datarc, 'dev')
        self.test_dataset = MTGGenreFeatureDataset(self.pre_extract_dir, self.datarc, 'test') if self.pre_extract_dir else MTGGenreAudioDataset(root_dir, self.datarc, 'test')
    ...
    def forward(self, mode, features, labels, filenames, records, **kwargs):
        ...
        # records['predict_genre'] += self.train_dataset.label2class(predicted_classid.cpu().tolist())
        # records['truth_genre'] += self.train_dataset.label2class(labels.cpu().tolist())
    ...
    def log_records(self, mode, records, logger, global_step, **kwargs):
        ...
        #         lines = [f"{f} {p}\n" for f, p in zip(records["filename"], records["predict_genre"])]
