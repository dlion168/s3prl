...
#-------------#
from ..model import *
from .dataset import MTGMoodAudioDataset, MTGMoodFeatureDataset
...
class DownstreamExpert(nn.Module):
    ...
    def __init__(self, upstream_dim, downstream_expert, expdir, **kwargs):
        ...
        self.train_dataset = MTGMoodFeatureDataset(self.pre_extract_dir, self.datarc, 'train') if self.pre_extract_dir else MTGMoodAudioDataset(root_dir, self.datarc, 'train', self.sample_rate)
        self.dev_dataset = MTGMoodFeatureDataset(self.pre_extract_dir, self.datarc, 'dev') if self.pre_extract_dir else MTGMoodAudioDataset(root_dir, self.datarc, 'dev', self.sample_rate)
        self.test_dataset = MTGMoodFeatureDataset(self.pre_extract_dir, self.datarc, 'test') if self.pre_extract_dir else MTGMoodAudioDataset(root_dir, self.datarc, 'test', self.sample_rate)
        ...
        self.objective = nn.BCEWithLogitsLoss() # nn.CrossEntropyLoss()
    ...
    def forward(self, mode, features, labels, filenames, records, **kwargs):
        ...
        records['logit'] += torch.sigmoid(predicted).cpu().float().tolist()
        records['labels'] += labels.cpu().float().tolist()
        records['loss'].append(loss.item())
        records['filename'] += filenames
        # records['predict_mood'] += self.train_dataset.label2class(predicted_classid.cpu().tolist())
        # records['truth_mood'] += self.train_dataset.label2class(labels.cpu().tolist())
    ...
    def log_records(self, mode, records, logger, global_step, **kwargs):
        ...
        for key, value in results.items():
            ...
            with open(Path(self.expdir) / "log.log", 'a') as f:
                ...
                if mode == 'dev' and key == 'roc_auc' and value > self.best_score:
        ...
        #         lines = [f"{f} {p}\n" for f, p in zip(records["filename"], records["predict_mood"])]
        #         file.writelines(lines)

        #     with open(Path(self.expdir) / f"{mode}_truth.txt", "w") as file:
        #         lines = [f"{f} {l}\n" for f, l in zip(records["filename"], records["truth_mood"])]
        #         file.writelines(lines)
