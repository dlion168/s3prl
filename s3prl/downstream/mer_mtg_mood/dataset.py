...
class MTGMoodAudioDataset(data.Dataset):
    def __init__(self, audio_dir, datarc, split, sample_rate, return_audio_path=True, split_version=0, low_quality_source=True): # Singer trim?
        # self.cfg = cfg
        if split == 'dev':
            split = 'validation'
        self.metadata_dir = os.path.join(datarc['meta_data'], f'data/splits/split-{split_version}/autotagging_moodtheme-{split}.tsv')
...
class MTGMoodFeatureDataset(data.Dataset):
