...
"""
    FileName     [ dataset.py ]
    Synopsis     [ The emotion classifier dataset, now also returns gender labels (with Race and AgeGroup) and always returns sample_weights. ]
"""
...
class WavSet(torch_utils.Dataset):
    """
    WavSet 現在也會儲存性別標籤（包含 Race 與 AgeGroup）。
    ...
    def __getitem__(self, idx):
        ...
        cur_gender = self.gender_list[idx]  # 格式： [Gender, K_Means, Race, AgeGroup]
...
def collate_fn_padd(batch):
    """
    現在也會處理 gender 標籤 (包含 Race 與 AgeGroup)。
    Batch 可能包含 5 個元素: (wav, lab, utt, dur, gender)
    或 4 個元素: (wav, lab, utt, gender) 取決於 print_dur。
    """
    total_wav = []
    total_lab = []
    total_utt = []
    ...
    for item in batch:
        # 無論是否有 dur，都固定取最後一項為 gender
        if len(item) == 5:
            wav, lab, utt, _, gender = item
        else:
...
class DataManager:
    ...
    def __load_msp_cat_label_dict__(self, label_path):
        ...
        gender_class_list = self.get_pseudo_gender_class()  # 現在回傳 4 個欄位

        with open(label_path, 'r') as f:
            header = f.readline().strip().split(",")
            emo_idx_list = [header.index(emo) for emo in emo_class_list]
            # 取得 Gender, K_Means, Race, AgeGroup 的欄位索引
            ...
            for row in csv_reader:
                ...
                # 轉換 Gender: Female -> 1.0, Male -> 0.0, 其他 -> -1.0
                gender_str = row[g_idx]
                if gender_str.lower() == "female":
                    ...
                    gender_label = -1.0
                # 轉換 K_Means 為 float
                k_means_val = float(row[pseudo_g_idx])
                # 轉換 Race (預設 mapping)
                race_str = row[race_idx]
                race_mapping = {"Asian": 0.0, "African American": 1.0, "Caucasian": 2.0, "Unknown": -1.0}
                race_label = race_mapping.get(race_str, -1.0)
                # 轉換 AgeGroup (預設 mapping)
                agegroup_str = row[agegroup_idx]
                agegroup_mapping = {"Young": 0.0, "Middle": 1.0, "Old": 2.0}
                agegroup_label = agegroup_mapping.get(agegroup_str, -1.0)
                discrete_labels = [gender_label, k_means_val, race_label, agegroup_label]
