...
# Usage example:
# sbatch run_downstream_sbatch.sh <distilled_model_checkpoint> <task> <stage> <current_row>
...
/workspace/s3prl/s3prl/run_inside_container_downstream.sh "$distilled_model_checkpoint" "$task" "$stage" "$current_row" "$upstream"