...
pip install networkx pytorch-nlp transformers datasets scipy==1.5.4 librosa==0.8.0 scikit-learn==0.24.2 matplotlib==3.3.4 modelscope==1.11.0
...
select_latest_checkpoint() {
  ...
  case $method in
    "list_based")
      ...
      ;;
    "hardcoded")
      echo "${CHECKPOINT_DIR}/averaged_model.ckpt"  # Default hardcoded checkpoint #averaged_model.ckpt
      ;;
    *)
...
if [ $task == "ks" ]; then
 ...
 cd $BASE_DIR_S3PRL
    ...
    echo "$stage $task"
      ...
      -o "config.downstream_expert.datarc.speech_commands_root=/livingrooms/public/superb3/speech_commands/train,,\
          config.downstream_expert.datarc.speech_commands_test_root=/livingrooms/public/superb3/speech_commands/test"
      echo "experiment finished so we will run the evaluation."
      echo "experiment finished so we will run the evaluation."
      echo "\n \n \n \n \n."
...
if [ $task == "vocalset_singer_id" ]; then
 ...
 cd $BASE_DIR_S3PRL
    ...
    echo "$stage $task"
      # Training (finetune on downstream task) # weighted sum of enc hdden states.
      python run_downstream.py -m $stage -c "./downstream/vocalset_singer_id/config.yaml" -u $upstream -k $latest_checkpoint $paper_arg --update_results \
